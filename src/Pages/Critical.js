...
export default function Critical() {
    ...
    const getPatients=()=>{
        fetch("http://151.106.113.197/get-patient-data?key=f4b1354db41f4c07fb33c5305c1fc3bd").then(res=>res.json()).then(data=>{
            console.log(data)
            data.forEach(a=>{
                if(a.emergency){
    ...
    return (
        <Box>
        {!loading && <Box> 
            <Box sx={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",width:"100vw", minHeight:"100vh",backgroundColor:"#f4f4f4",marginTop:{xs:"0",md:"7vh"}}}>
